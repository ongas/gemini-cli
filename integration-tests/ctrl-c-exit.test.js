...
describe('Ctrl+C exit', () => {
    it('should exit gracefully on second Ctrl+C', async () => {
        ...
        const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error(`Test timed out: process did not exit within a minute. Output: ${output}`)), 60000));
        ...
        if (os.platform() === 'win32') {
            ...
            expect(result.exitCode, `Process exited with code ${result.exitCode}. Output: ${result.output}`).not.toBeNull();
        }
        else {
