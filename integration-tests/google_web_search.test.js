...
describe('google_web_search', () => {
    it('should be able to search the web', async () => {
        ...
        if (!foundToolCall) {
            ...
            const failedSearchCalls = allTools.filter((t) => t.toolRequest.name === 'google_web_search' && !t.toolRequest.success);
            if (failedSearchCalls.length > 0) {
                console.warn('google_web_search tool was called but failed, possibly due to network issues');
                console.warn('Failed calls:', failedSearchCalls.map((t) => t.toolRequest.args));
                return; // Skip the test if network issues
            }
        }
        expect(foundToolCall, 'Expected to find a call to google_web_search').toBeTruthy();
