...
describe('JSON output', () => {
    ...
    it('should return a JSON error for enforced auth mismatch before running', async () => {
        ...
        expect(payload.error.message).toContain('configured auth type is gemini-api-key');
        expect(payload.error.message).toContain('current auth type is oauth-personal');
    });
});
