...
describe('replace', () => {
    ...
    it('should fail safely when old_string is not found', async () => {
        ...
        const replaceAttempt = toolLogs.find((log) => log.toolRequest.name === 'replace');
        ...
        expect(readAttempt || replaceAttempt, 'Expected model to attempt a read_file or replace').toBeDefined();
        ...
        if (replaceAttempt) {
            ...
            expect(replaceAttempt.toolRequest.success, 'If replace is called, it must fail').toBe(false);
        }
