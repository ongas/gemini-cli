...
describe('replace', () => {
    ...
    it.skip('should handle $ literally when replacing text ending with $', async () => {
        ...
        rig.createFile(fileName, originalContent);
        await rig.run("Open regex.yml and append ' # updated' after the line containing ^[sv]d[a-z]$ without breaking the $ character.");
        const foundToolCall = await rig.waitForToolCall('replace');
        expect(foundToolCall, 'Expected to find a replace tool call').toBeTruthy();
        expect(rig.readFile(fileName)).toBe(expectedContent);
    });
    ...
    it.skip('should delete a block of text', async () => {
        ...
        rig.createFile(fileName, originalContent);
        await rig.run(`In ${fileName}, delete the entire block from "## DELETE THIS ##" to "## END DELETE ##" including the markers and the newline that follows it.`);
        const foundToolCall = await rig.waitForToolCall('replace');
        expect(foundToolCall, 'Expected to find a replace tool call').toBeTruthy();
        expect(rig.readFile(fileName)).toBe(expectedContent);
    });
