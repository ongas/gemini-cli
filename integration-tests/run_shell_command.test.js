...
describe('run_shell_command', () => {
    ...
    it.skip('should combine multiple --allowed-tools flags', async () => {
        ...
        const result = await rig.run({
            stdin: prompt,
            yolo: false,
        }, `--allowed-tools=run_shell_command(${tool})`, '--allowed-tools=run_shell_command(ls)');
    ...
    it('should allow all with "ShellTool" and other specific tools', async () => {
        ...
        const result = await rig.run({
            stdin: prompt,
            yolo: false,
        }, `--allowed-tools=run_shell_command(${tool})`, '--allowed-tools=run_shell_command');
        const foundToolCall = await rig.waitForToolCall('run_shell_command', 15000);
    ...
    it('rejects invalid shell expressions', async () => {
        ...
        const result = await rig.run(`I am testing the error handling of the run_shell_command tool. Please attempt to run the following command, which I know has invalid syntax: \`${invalidCommand}\`. If the command fails as expected, please return the word FAIL, otherwise return the word SUCCESS.`);
        expect(result).toContain('FAIL');
