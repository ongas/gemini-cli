...
import { AuthType, Config, FileDiscoveryService, ApprovalMode, loadServerHierarchicalMemory, GEMINI_DIR, DEFAULT_GEMINI_EMBEDDING_MODEL, DEFAULT_GEMINI_MODEL, debugLogger, } from '@google/gemini-cli-core';
...
export async function loadConfig(settings, extensions, taskId) {
    ...
    const { memoryContent, fileCount } = await loadServerHierarchicalMemory(workspaceDir, [workspaceDir], false, fileService, extensions, settings.folderTrust === true);
...
export function mergeMcpServers(settings, extensions) {
    ...
    for (const extension of extensions) {
        Object.entries(extension.mcpServers || {}).forEach(([key, server]) => {
            if (mcpServers[key]) {
                debugLogger.warn(`Skipping extension MCP config for server with key "${key}" as it already exists.`);
                return;
            }
