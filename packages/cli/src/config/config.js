...
export async function parseArguments(settings) {
    ...
    const yargsInstance = yargs(rawArgv)
        ...
        type: 'string',
        description: 'Continue a previous conversation session by tag name. Loads conversation history and appends new messages.',
    })
        .option('init', {
        type: 'boolean',
        description: 'Initialize gemini-cli project structure: creates .gemini/standards/ (coding standards) and .gemini/agents/ (custom agents) directories in the current directory.',
    })
        .option('list-agents', {
        type: 'boolean',
        description: 'List all available agents with their descriptions, providers, and models.',
    })
        .option('agent', {
        type: 'string',
        description: 'Use a specific agent for this session. Agent name should match a file in .gemini/agents/ (without .md extension, use underscores for hyphens).',
    })
...
export async function loadCliConfig(settings, extensions, extensionEnablementManager, sessionId, argv, cwd = process.cwd()) {
    ...
    const extraExcludes = [];
    const messageBusEnabled = settings.tools?.enableMessageBusIntegration ?? false;
 ...
 */
async function shouldEnableSubagents(settings, cwd, argv) {
