...
import { ShellTool, EditTool, WriteFileTool, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_MODEL_AUTO, OutputFormat, } from '@google/gemini-cli-core';
...
describe('loadCliConfig', () => {
    ...
    it('should set showMemoryUsage to true when --show-memory-usage flag is present', async () => {
        ...
        const argv = await parseArguments({});
        const settings = {};
        const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    it('should pass extension context file paths to loadServerHierarchicalMemory', async () => {
        ...
        const argv = await parseArguments({});
        await loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'session-id', argv);
...
describe('mergeMcpServers', () => {
    it('should not modify the original settings object', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        await loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('mergeExcludeTools', () => {
    ...
    it('should merge excludeTools from settings and extensions', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('Approval mode tool exclusion logic', () => {
    ...
    it('should throw an error for invalid approval mode values in loadCliConfig', async () => {
        ...
        const settings = {};
        const extensions = [];
        await expect(loadCliConfig(settings, extensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), invalidArgv.extensions), 'test-session', invalidArgv)).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: yolo, auto_edit, default');
    });
});
describe('loadCliConfig with allowed-mcp-server-names', () => {
    ...
    it('should allow all MCP servers if the flag is not provided', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig(baseSettings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('loadCliConfig extensions', () => {
    ...
    it('should not filter extensions if --extensions flag is not used', async () => {
        ...
        const argv = await parseArguments({});
        const settings = {};
        const config = await loadCliConfig(settings, mockExtensions, new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('loadCliConfig model selection', () => {
    it('selects a model from settings.json if provided', async () => {
        ...
        }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('loadCliConfig useRipgrep', () => {
    ...
    describe('loadCliConfig enableSubagents', () => {
        ...
        it('should be true when enableSubagents is set to true in settings', async () => {
            process.argv = ['node', 'script.js'];
            const argv = await parseArguments({});
            const settings = { experimental: { enableSubagents: true } };
            const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
        ...
        it('should be false when enableSubagents is explicitly set to false in settings', async () => {
            process.argv = ['node', 'script.js'];
            const argv = await parseArguments({});
            const settings = { experimental: { enableSubagents: false } };
            const config = await loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
            expect(config.getEnableSubagents()).toBe(false);
        });
    });
...
describe('loadCliConfig tool exclusions', () => {
    ...
    it('should not exclude interactive tools in interactive mode without YOLO', async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({}, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('Output format', () => {
    ...
    it('should use the format from settings', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({ output: { format: OutputFormat.JSON } }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
...
describe('Telemetry configuration via environment variables', () => {
    ...
    it('should throw when GEMINI_TELEMETRY_TARGET is invalid', async () => {
        ...
        await expect(loadCliConfig(settings, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv)).rejects.toThrow(/Invalid telemetry configuration: .*Invalid telemetry target/i);
        vi.unstubAllEnvs();
    });
    ...
    it("should treat GEMINI_TELEMETRY_ENABLED='0' as false", async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({ telemetry: { enabled: true } }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
        expect(config.getTelemetryEnabled()).toBe(false);
    });
    ...
    it("should treat GEMINI_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments({});
        const config = await loadCliConfig({ telemetry: { logPrompts: true } }, [], new ExtensionEnablementManager(ExtensionStorage.getUserExtensionsDir(), argv.extensions), 'test-session', argv);
        expect(config.getTelemetryLogPromptsEnabled()).toBe(false);
    });
});
