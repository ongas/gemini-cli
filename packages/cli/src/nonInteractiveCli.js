...
import { executeToolCall, shutdownTelemetry, isTelemetrySdkInitialized, GeminiEventType, FatalInputError, promptIdContext, OutputFormat, JsonFormatter, uiTelemetryService, Logger, } from '@google/gemini-cli-core';
...
export async function runNonInteractive(config, settings, input, prompt_id, continueSession) {
                ...
                try {
                    ...
                    if (loadedHistory && loadedHistory.length > 0) {
                        ...
                        if (config.getDebugMode()) {
                            console.log(`[DEBUG] Loaded ${loadedHistory.length} messages from session '${continueSession}'`);
                        }
                    }
                }
                catch (error) {
                    if (config.getDebugMode()) {
                        console.warn(`[DEBUG] Could not load session '${continueSession}':`, error);
                    }
                }
            }
            ...
            while (true) {
                ...
                if (toolCallRequests.length > 0) {
                    ...
                    try {
                        ...
                        if (config.getDebugMode()) {
                            console.log(`[DEBUG] Saved ${currentHistory.length} messages to session '${sessionTag}'`);
                        }
                    }
                    catch (error) {
                        if (config.getDebugMode()) {
                            console.warn(`[DEBUG] Could not save session '${sessionTag}':`, error);
                        }
                    }
                    if (config.getOutputFormat() === OutputFormat.JSON) {
