 ...
 */
export async function runAgentNonInteractive(config, _settings, agentFilePath, userTask) {
    try {
        // Load the agent definition from markdown file
        const agentDefinition = await MarkdownAgentLoader.loadFromFile(agentFilePath, config.getDebugMode());
        if (!agentDefinition) {
        ...
        console.log(`   Task: ${userTask.slice(0, 100)}${userTask.length > 100 ? '...' : ''}\n`);
        // Validate tools if specified
        if (agentDefinition.toolConfig?.tools) {
            const toolRegistry = await config.getToolRegistry();
            const availableTools = toolRegistry.getAllToolNames();
            const requestedTools = agentDefinition.toolConfig.tools.filter((t) => typeof t === 'string');
            const invalidTools = requestedTools.filter((t) => !availableTools.includes(t));
            ...
            if (config.getDebugMode()) {
                console.log(`[Agent Tools] Validated ${requestedTools.length} tools: ${requestedTools.join(', ')}`);
            }
        }
        ...
        const onActivity = (activity) => {
            ...
            if (activity.type === 'TOOL_CALL_START') {
                ...
                console.error(`❌ Error: ${activity.data.error || JSON.stringify(activity.data)}`);
            }
        };
        // Create the agent executor
        const executor = await AgentExecutor.create(agentDefinition, config, onActivity);
        ...
        // Run the agent
        const result = await executor.run({ task: userTask }, abortController.signal);
        // Display results
        console.log('\n' + '='.repeat(70));
