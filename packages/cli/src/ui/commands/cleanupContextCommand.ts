...
export const cleanupContextCommand: SlashCommand = {
  name: 'cleanup-context',
  altNames: ['cleanup', 'trim-context'],
  description:
    'Aggressively cleans up conversation context by removing old tool results and trimming history. Keeps only recent conversation turns.',
  ...
  action: async (context) => {
    ...
    try {
      ...
      const turnsRemoved = Math.floor(
        (history.length - cleanedHistory.length) / 2,
      );
      ...
      ui.addItem(
        {
          type: MessageType.ERROR,
          text: `Failed to cleanup context: ${
