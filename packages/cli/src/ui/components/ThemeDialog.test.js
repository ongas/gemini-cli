...
describe('ThemeDialog Snapshots', () => {
    ...
    it('should call onCancel when ESC is pressed', async () => {
        ...
        const settings = createMockSettings();
        const { stdin } = render(_jsx(SettingsContext.Provider, { value: settings, children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(ThemeDialog, { ...baseProps, onCancel: mockOnCancel, settings: settings }) }) }));
        act(() => {
            stdin.write('\x1b');
        });
        await waitFor(() => {
