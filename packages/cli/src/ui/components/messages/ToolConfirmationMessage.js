...
export const ToolConfirmationMessage = ({ confirmationDetails, config, isFocused = true, availableTerminalHeight, terminalWidth, }) => {
    ...
    const handleConfirm = async (outcome) => {
        ...
        fs.appendFileSync('/tmp/gemini-ui-debug.log', `${new Date().toISOString()} - handleConfirm called with outcome: ${outcome}\n`);
        if (confirmationDetails.type === 'edit') {
            fs.appendFileSync('/tmp/gemini-ui-debug.log', `${new Date().toISOString()} - Confirmation type is 'edit'\n`);
        ...
        fs.appendFileSync('/tmp/gemini-ui-debug.log', `${new Date().toISOString()} - About to call onConfirm\n`);
        try {
            await onConfirm(outcome);
            fs.appendFileSync('/tmp/gemini-ui-debug.log', `${new Date().toISOString()} - onConfirm completed successfully\n`);
        }
        catch (error) {
            fs.appendFileSync('/tmp/gemini-ui-debug.log', `${new Date().toISOString()} - onConfirm ERROR: ${error instanceof Error ? error.message : String(error)}\n`);
            fs.appendFileSync('/tmp/gemini-ui-debug.log', `${new Date().toISOString()} - onConfirm ERROR stack: ${error instanceof Error ? error.stack : 'no stack'}\n`);
            throw error;
        }
    };
    ...
    if (confirmationDetails.type === 'edit') {
        if (confirmationDetails.isModifying) {
            return (_jsxs(Box, { minWidth: "90%", borderStyle: "round", borderColor: theme.border.default, justifyContent: "space-around", padding: 1, overflow: "hidden", children: [_jsx(Text, { color: theme.text.primary, children: "Modify in progress: " }), _jsx(Text, { color: theme.status.success, children: "Save and close external editor to continue" })] }));
        }
        ...
        bodyContent = (_jsxs(Box, { flexDirection: "column", paddingX: 1, marginLeft: 1, children: [_jsxs(Text, { color: theme.text.link, children: ["MCP Server: ", mcpProps.serverName] }), _jsxs(Text, { color: theme.text.link, children: ["Tool: ", mcpProps.toolName] })] }));
