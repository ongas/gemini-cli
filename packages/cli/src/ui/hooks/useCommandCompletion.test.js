...
describe('useCommandCompletion', () => {
    ...
    describe('Core Hook Behavior', () => {
        describe('State Management', () => {
            it('should initialize with default state', () => {
                const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest(''), testDirs, testRootDir, [], mockCommandContext, false, false, mockConfig));
            ...
            it('should reset all state to default values', () => {
                const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest('@files'), testDirs, testRootDir, [], mockCommandContext, false, false, mockConfig));
                act(() => {
            ...
            it('should call useAtCompletion with the correct query for an escaped space', async () => {
                ...
                renderHook(() => useCommandCompletion(useTextBufferForTest(text), testDirs, testRootDir, [], mockCommandContext, false, false, mockConfig));
                await waitFor(() => {
            ...
            it('should correctly identify the completion context with multiple @ symbols', async () => {
                ...
                renderHook(() => useCommandCompletion(useTextBufferForTest(text, cursorOffset), testDirs, testRootDir, [], mockCommandContext, false, false, mockConfig));
                await waitFor(() => {
                    expect(useAtCompletion).toHaveBeenLastCalledWith(expect.objectContaining({
                        enabled: true,
                        pattern: 'file1',
            ...
            it.each([
                ...
                const { result } = renderHook(() => {
                    ...
                    const completion = useCommandCompletion(textBuffer, testDirs, testRootDir, [], mockCommandContext, false, shellModeActive, // Parameterized shellModeActive
                    mockConfig);
                    return { ...completion, textBuffer };
                });
                await waitFor(() => {
    ...
    describe('prompt completion filtering', () => {
        ...
        it('should trigger prompt completion for regular text when enabled', async () => {
            ...
            expect(result.current.textBuffer.text).toBe('This is regular text that should trigger completion');
        });
    });
});
