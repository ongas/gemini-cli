...
export const useExtensionUpdates = (extensions, addItem, cwd) => {
    ...
    useEffect(() => {
        (async () => {
            await checkForAllExtensionUpdates(extensions, dispatchExtensionStateUpdate);
        })();
    ...
    useEffect(() => {
        ...
        for (const extension of extensions) {
            ...
            if (extension.installMetadata?.autoUpdate) {
                updateExtension(extension, cwd, (description) => requestConsentInteractive(description, addConfirmUpdateExtensionRequest), currentState.status, dispatchExtensionStateUpdate)
                    .then((result) => {
                    if (!result)
                        return;
                    addItem({
