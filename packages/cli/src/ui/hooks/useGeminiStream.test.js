...
import { ApprovalMode, AuthType, GeminiEventType as ServerGeminiEventType, ToolErrorType, ToolConfirmationOutcome, } from '@google/gemini-cli-core';
...
describe('useGeminiStream', () => {
    ...
    describe('handleApprovalModeChange', () => {
        ...
        it('should not auto-approve any tools when switching to REQUIRE_CONFIRMATION mode', async () => {
            ...
            const { result } = renderTestHook(awaitingApprovalToolCalls);
            await act(async () => {
                await result.current.handleApprovalModeChange(ApprovalMode.REQUIRE_CONFIRMATION);
            });
