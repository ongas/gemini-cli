...
vi.mock('readline', () => {
    const mockedReadline = {
        ...
        emitKeypressEvents: vi.fn((stream) => {
...
describe('useKeypress', () => {
    ...
    const wrapper = ({ children }) => React.createElement(KeypressProvider, null, children);
    beforeEach(() => {
        vi.clearAllMocks();
        stdin = new MockStdin();
    ...
    it('should set and release raw mode', () => {
        const { unmount } = renderHook(() => useKeypress(onKeypress, { isActive: true }), { wrapper });
