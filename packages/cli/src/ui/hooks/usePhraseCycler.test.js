...
describe('usePhraseCycler', () => {
    ...
    it('should show "Waiting for user confirmation..." when isWaiting is true', () => {
        const { result, rerender } = renderHook(({ isActive, isWaiting }) => usePhraseCycler(isActive, isWaiting), { initialProps: { isActive: true, isWaiting: false } });
    ...
    it('should reset to a witty phrase when isActive becomes true after being false (and not waiting)', () => {
        ...
        const { result, rerender } = renderHook(({ isActive, isWaiting }) => usePhraseCycler(isActive, isWaiting), { initialProps: { isActive: false, isWaiting: false } });
        // Activate
    ...
    it('should use custom phrases when provided', () => {
        ...
        const { result, rerender } = renderHook(({ isActive, isWaiting, customPhrases: phrases }) => usePhraseCycler(isActive, isWaiting, phrases), {
            initialProps: {
                isActive: true,
                isWaiting: false,
                customPhrases,
    ...
    it('should fall back to witty phrases if custom phrases are an empty array', () => {
        const { result } = renderHook(({ isActive, isWaiting, customPhrases: phrases }) => usePhraseCycler(isActive, isWaiting, phrases), {
            initialProps: {
                isActive: true,
                isWaiting: false,
                customPhrases: [],
