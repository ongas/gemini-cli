...
import { UserTierId, AuthType, TerminalQuotaError, makeFakeConfig, } from '@google/gemini-cli-core';
    ...
    describe('Fallback Handler Logic', () => {
        ...
        describe('Automatic Fallback Scenarios', () => {
            ...
            for (const { errorType, tier, expectedMessageSnippets } of testCases) {
                it(`should handle ${errorType} error for ${tier} tier correctly`, async () => {
                    ...
                    const result = await handler('model-A', 'model-B', new Error('some error'));
        ...
        describe('Interactive Fallback (Pro Quota Error)', () => {
            it('should set an interactive request and wait for user choice', async () => {
                ...
                const promise = handler('gemini-pro', 'gemini-flash', new TerminalQuotaError('pro quota', mockGoogleApiError));
                await act(async () => { });
            ...
            it('should handle race conditions by stopping subsequent requests', async () => {
                ...
                const handler = setFallbackHandlerSpy.mock
                    .calls[0][0];
                const promise1 = handler('gemini-pro', 'gemini-flash', new TerminalQuotaError('pro quota 1', mockGoogleApiError));
                await act(async () => { });
                const firstRequest = result.current.proQuotaRequest;
                expect(firstRequest).not.toBeNull();
                const result2 = await handler('gemini-pro', 'gemini-flash', new TerminalQuotaError('pro quota 2', mockGoogleApiError));
