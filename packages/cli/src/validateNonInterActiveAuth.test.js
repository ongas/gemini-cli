...
describe('validateNonInterActiveAuth', () => {
    ...
    it('exits if currentAuthType does not match enforcedAuthType', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('The configured auth type is oauth-personal, but the current auth type is vertex-ai. Please re-authenticate with the correct type.');
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    describe('JSON output mode', () => {
        ...
        it('prints JSON error when enforced auth mismatches current auth and exits with code 1', async () => {
            ...
            expect(thrown?.message).toBe('process.exit(1) called');
            {
                ...
                expect(payload.error.type).toBe('Error');
                expect(payload.error.code).toBe(1);
                expect(payload.error.message).toContain('The configured auth type is gemini-api-key, but the current auth type is oauth-personal.');
            }
        });
