...
export class AgentRegistry {
    ...
    async initialize() {
        ...
        if (debugMode) {
            ...
            for (const agent of this.agents.values()) {
                ...
                console.log(`  - ${agent.displayName || agent.name} (${provider}/${agent.modelConfig.model})`);
            }
        }
    }
    loadBuiltInAgents() {
    ...
    async loadMarkdownAgents() {
        ...
        try {
            ...
            if (debugMode) {
                console.log(`[AgentRegistry] Loading markdown agents from ${agentsDir}`);
            }
            const agents = await MarkdownAgentLoader.loadFromDirectory(agentsDir, debugMode);
            for (const agent of agents) {
                this.registerAgent(agent);
            }
            if (debugMode && agents.length > 0) {
                console.log(`[AgentRegistry] Loaded ${agents.length} markdown agents from ${agentsDir}`);
            }
        }
        catch {
            // Directory doesn't exist or can't be accessed - this is fine, not all projects will have custom agents
            if (debugMode) {
                console.log(`[AgentRegistry] No markdown agents directory found at ${agentsDir}`);
            }
        }
    }
    /**
