...
import { BrowserActionTool } from '../tools/browser-action.js';
...
export class Config {
    ...
    async createToolRegistry() {
        ...
        const registerCoreTool = (ToolClass, ...args) => {
            ...
            if (agentTools) {
                isEnabled = agentTools.some((tool) => tool === toolName || tool === normalizedClassName);
                console.log(`[AGENT TOOLS DEBUG] Checking ${toolName}: agentTools=[${agentTools.join(',')}], isEnabled=${isEnabled}`);
            }
            else if (coreTools) {
        ...
        registerCoreTool(BrowserActionTool, this);
        registerCoreTool(TaskTool, this);
        if (this.getUseWriteTodos()) {
            registerCoreTool(WriteTodosTool, this);
        }
        // Register Subagents as Tools
        console.log(`[Config] EnableSubagents: ${this.getEnableSubagents()}, DebugMode: ${this.getDebugMode()}`);
        if (this.getEnableSubagents()) {
            const agentDefinitions = this.agentRegistry.getAllDefinitions();
            console.log(`[Config] Registering ${agentDefinitions.length} agents as tools...`);
            for (const definition of agentDefinitions) {
                ...
                if (this.getDebugMode()) {
                    console.log(`[Config] Agent '${definition.name}': isAllowed=${isAllowed}, isExcluded=${isExcluded}, allowedTools=${allowedTools?.join(',')}`);
                }
                if (isAllowed && !isExcluded) {
                    try {
                        ...
                        const wrapper = new SubagentToolWrapper(definition, this, this.getMessageBus());
                        registry.registerTool(wrapper);
                        if (this.getDebugMode()) {
                            console.log(`[Config] Successfully registered agent '${definition.name}' as tool`);
                        }
                    }
                    catch (error) {
