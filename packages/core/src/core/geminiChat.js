...
export class GeminiChat {
    ...
    async sendMessageStream(model, params, prompt_id) {
        ...
        if (Array.isArray(params.message)) {
            ...
            if (textParts.length > 0) {
                ...
                if (typeof textParts[0] === 'string') {
                    ...
                    messageWithContext = params.message.map((part) => part === textParts[0] ? { ...part, text: textWithContext } : part);
                }
            }
        }
        ...
        const { trimmedHistory, warning, breakdown } = checkAndTrimContext(model, requestContents, this.generationConfig.systemInstruction, this.generationConfig.tools);
                    ...
                    try {
                        ...
                        if (isEmptyResponseError && self.config.isInFallbackMode()) {
                            ...
                            if (self.consecutiveEmptyResponses >= 2) {
                                console.log('[RETRY DEBUG] Detected pattern of persistent empty responses, stopping retries early');
                                console.log('[RETRY DEBUG] This suggests the model cannot handle this request in fallback mode');
        ...
        if (!hasToolCall && (!hasFinishReason || !responseText)) {
            if (!hasFinishReason) {
                ...
                if (isOllama) {
                    ...
                    throw new InvalidStreamError(`❌ LOCAL LLM ERROR: Stream timed out without receiving any response\n\n` +
                        ...
                        `     • Increasing server timeout settings if available`, 'NO_FINISH_REASON');
                }
                ...
                if (this.config.isInFallbackMode()) {
                    throw new InvalidStreamError('Flash model returned empty response (likely quota exhausted).\n\n' +
                        ...
                        'Please wait a moment and send your message again manually.', 'NO_RESPONSE_TEXT', lastFinishReason, true);
                }
