    ...
    return new Promise((resolve, reject) => {
        const scheduler = new CoreToolScheduler({
            ...
            onToolCallsUpdate: async (toolCalls) => {
                ...
                const waitingTool = toolCalls.find((tc) => tc.status === 'awaiting_approval');
                if (waitingTool && waitingTool.status === 'awaiting_approval') {
                    ...
                    await scheduler.handleConfirmationResponse(waitingTool.request.callId, confirmationDetails.onConfirm, outcome, abortSignal);
                }
            },
        });
