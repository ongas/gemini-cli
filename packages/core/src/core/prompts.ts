...
${(function () {
  if (!config.isInteractiveShellEnabled()) {
    ...
    return `- **Interactive Commands & Password Prompts:** Prefer non-interactive commands when it makes sense; however, some commands are only interactive and expect user input during their execution (e.g. ssh, vim, sudo). Password prompts from sudo commands are automatically detected and the user will be notified to press \`ctrl + f\` to focus the terminal and enter their password. For other interactive commands, you can inform the user they can press \`ctrl + f\` to provide input if needed.`;
  }
})()}
