...
export class ShellToolInvocation extends BaseToolInvocation {
    ...
    async shouldConfirmExecute(_abortSignal) {
        ...
        const persistentlyApprovedCommands = new Set();
            ...
            onConfirm: async (outcome) => {
                if (outcome === ToolConfirmationOutcome.ProceedAlways) {
                    ...
                    for (const command of commandsToConfirm) {
                        try {
                            await approvalStorage.approveCommand(command, `Shell command: ${command}`);
                            console.log(`[DEBUG] Persistent approval saved for shell command: ${command}`);
                            // Also add to session allowlist
                            this.allowlist.add(command);
                        }
                        catch (error) {
                            console.error(`[ERROR] Failed to save persistent approval for ${command}:`, error);
