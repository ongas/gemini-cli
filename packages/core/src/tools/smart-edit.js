...
import { detectIndentationStyle, normalizeIndentation, validatePythonSyntax, tryFixPythonIndentation, } from '../utils/indentationNormalizer.js';
 ...
 */
async function calculateNormalizedReplacement(context) {
    const { currentContent, params } = context;
    const { old_string, new_string } = params;
    ...
    if (normalizedOldString === old_string &&
    ...
    if (exactOccurrences > 0) {
        let modifiedCode = safeLiteralReplace(normalizedCode, normalizedOldString, normalizedNewString);
        modifiedCode = restoreTrailingNewline(currentContent, modifiedCode);
        return {
            newContent: modifiedCode,
            occurrences: exactOccurrences,
    ...
    async calculateEdit(params, abortSignal) {
        ...
        if (!initialError) {
            ...
            if (params.file_path.endsWith('.py')) {
                const syntaxError = await validatePythonSyntax(replacementResult.newContent);
                if (syntaxError && syntaxError.includes('IndentationError')) {
                    // Try to fix the indentation automatically
                    const fileIndentStyle = detectIndentationStyle(currentContent);
                    const fixedContent = await tryFixPythonIndentation(replacementResult.newContent, fileIndentStyle);
