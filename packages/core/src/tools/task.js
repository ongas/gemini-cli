...
const taskToolSchemaData = {
    ...
    parametersJsonSchema: {
        ...
        properties: {
            description: {
                type: 'string',
                description: 'A concise (3-5 word) description of the task for tracking purposes (e.g., "Fix authentication bug", "Analyze performance bottleneck")',
            },
            prompt: {
                type: 'string',
                description: 'The detailed task prompt for the sub-agent. Be specific about what needs to be accomplished, any constraints, and what information should be returned. The sub-agent will work autonomously based on this prompt.',
            },
            tools: {
                type: 'array',
                items: { type: 'string' },
                description: 'Optional array of tool names the sub-agent is allowed to use (e.g., ["read_file", "write_file", "run_shell_command"]). If not specified, the sub-agent will have access to all available tools.',
            },
            max_time_minutes: {
                type: 'number',
                description: 'Optional maximum execution time in minutes (default: 10). The sub-agent will be terminated if it exceeds this duration.',
            },
            max_turns: {
                type: 'number',
                description: 'Optional maximum number of conversation turns (default: 50). The sub-agent will be terminated if it exceeds this limit.',
            },
        },
...
class TaskToolInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        const { description, prompt, tools, max_time_minutes = 10, max_turns = 50, } = this.params;
        try {
            ...
            const scope = await SubAgentScope.create(description, this.config, promptConfig, modelConfig, runConfig, {
                toolConfig,
                onMessage,
            });
            // Run sub-agent
            ...
            console.error(`[TaskTool] Error executing task "${description}": ${errorMessage}`);
            return {
...
export class TaskTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(TaskTool.Name, 'Task', taskToolDescription, Kind.Think, taskToolSchemaData.parametersJsonSchema);
        this.config = config;
    }
