...
import { findPythonMatchWithFlexibleIndentation, isPythonFile, applyPythonMatch, } from './pythonIndentMatcher.js';
...
const failedEditAttempts = new LruCache(MAX_FAILED_ATTEMPTS_TRACKED);
/**
 * Records a failed edit attempt for tracking purposes.
 */
function recordFailedEditAttempt(filePath, oldString) {
    const now = Date.now();
    const key = filePath;
    // Get existing attempts for this file
    let attempts = failedEditAttempts.get(key) || [];
    // Clean up old attempts outside the time window
    attempts = attempts.filter((attempt) => now - attempt.timestamp < FAILED_EDIT_WINDOW_MS);
    ...
    const similarAttempts = attempts.filter((attempt) => attempt.oldString.substring(0, 100).includes(oldStringPrefix) ||
 ...
 */
function getFailedAttemptInfo(filePath, oldString) {
    const now = Date.now();
    const key = filePath;
    const attempts = failedEditAttempts.get(key) || [];
    const recentAttempts = attempts.filter((attempt) => now - attempt.timestamp < FAILED_EDIT_WINDOW_MS);
    if (recentAttempts.length === 0) {
        return { recentAttempts: 0, shouldStop: false, errorMessage: null };
    }
    // Check for similar attempts
    const oldStringPrefix = oldString.substring(0, 100);
    const similarAttempts = recentAttempts.filter((attempt) => attempt.oldString.substring(0, 100).includes(oldStringPrefix) ||
        oldStringPrefix.includes(attempt.oldString.substring(0, 100)));
    ...
    if (occurrences === expectedReplacements) {
        ...
        if (occurrences === expectedReplacements) {
            ...
            if (filePath && isPythonFile(filePath)) {
                console.log(`[Python Edit] Attempting flexible indentation matching for ${filePath}`);
                console.log(`[Python Edit] old_string has ${unescapedOldStringAttempt.split('\n').length} lines`);
                console.log(`[Python Edit] File has ${currentContent.split('\n').length} total lines`);
                const pythonMatch = findPythonMatchWithFlexibleIndentation(currentContent, unescapedOldStringAttempt, originalParams.new_string, true);
                if (pythonMatch) {
                    const { actualOldString, actualNewString } = applyPythonMatch(currentContent, pythonMatch);
                    ...
                    console.log(`[Python Edit] ✓ Found match with flexible indentation adjustment (base indent: ${pythonMatch.fileIndent} spaces)`);
                    const result = {
                        params: {
                    ...
                    console.log(`[Python Edit] ✗ No match found with flexible indentation`);
                    console.log(`[Python Edit] First line of old_string (trimmed): "${unescapedOldStringAttempt.split('\n')[0].trim()}"`);
                    console.log(`[Python Edit] Last line of old_string (trimmed): "${unescapedOldStringAttempt.split('\n').slice(-1)[0].trim()}"`);
                }
            }
            if (filePath) {
            ...
            console.log(`[Edit Corrector] Attempting LLM-based old_string correction for ${filePath}`);
