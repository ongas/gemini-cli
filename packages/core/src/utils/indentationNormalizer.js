...
export function normalizeIndentation(text, targetStyle) {
    ...
    if (sourceStyle.char === targetStyle.char &&
    ...
    const normalizedLines = lines.map((line) => {
        ...
        const leadingWhitespace = line.match(/^(\s+)/);
        if (!leadingWhitespace)
            return line; // No indentation to fix
        const whitespace = leadingWhitespace[1];
        ...
        const newIndent = targetStyle.char === '\t'
            ? '\t'.repeat(indentLevel)
...
export async function validatePythonSyntax(code, timeout = 500) {
    return new Promise((resolve) => {
        const pythonProcess = spawn('python3', ['-c', `import ast; ast.parse(${JSON.stringify(code)})`], {
            timeout,
