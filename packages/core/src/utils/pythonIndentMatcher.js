 ...
 */
function analyzeIndentationPattern(text) {
 ...
 */
function adjustIndentation(newString, oldBaseIndent, fileBaseIndent) {
 ...
 */
export function findPythonMatchWithFlexibleIndentation(fileContent, oldString, newString, debug = false) {
    const fileLines = fileContent.split('\n');
    const { baseIndent: oldBaseIndent, pattern: oldPattern } = analyzeIndentationPattern(oldString);
    if (debug) {
        console.log(`[Python Matcher] Searching for pattern with ${oldPattern.length} lines`);
        console.log(`[Python Matcher] old_string base indent: ${oldBaseIndent} spaces`);
        console.log(`[Python Matcher] First line pattern: "${oldPattern[0].content}" (relative indent: ${oldPattern[0].relativeIndent})`);
        if (oldPattern.length > 1) {
            console.log(`[Python Matcher] Last line pattern: "${oldPattern[oldPattern.length - 1].content}" (relative indent: ${oldPattern[oldPattern.length - 1].relativeIndent})`);
        }
    }
    ...
    for (let i = 0; i <= fileLines.length - oldPattern.length; i++) {
        ...
        const { baseIndent: candidateBaseIndent, pattern: candidatePattern } = analyzeIndentationPattern(candidateText);
        // Track best partial match for debugging
        if (debug) {
            let matchingLines = 0;
            for (let j = 0; j < Math.min(oldPattern.length, candidatePattern.length); j++) {
                if (oldPattern[j].content === candidatePattern[j].content &&
        ...
        if (patternsMatch(oldPattern, candidatePattern)) {
            ...
            const adjustedNewString = adjustIndentation(newString, oldBaseIndent, candidateBaseIndent);
            if (debug) {
                console.log(`[Python Matcher] ✓ Found exact match at line ${i + 1} (file indent: ${candidateBaseIndent} spaces)`);
            }
            return {
                startLine: i,
    ...
    if (debug && bestPartialMatch > 0) {
        console.log(`[Python Matcher] Best partial match: ${bestPartialMatch}/${oldPattern.length} lines at line ${bestPartialMatchLine + 1}`);
        // Show where the match diverged
        const candidateLines = fileLines.slice(bestPartialMatchLine, bestPartialMatchLine + oldPattern.length);
        const candidateText = candidateLines.join('\n');
        const { pattern: candidatePattern } = analyzeIndentationPattern(candidateText);
        for (let j = 0; j < oldPattern.length; j++) {
            if (j >= candidatePattern.length ||
                oldPattern[j].content !== candidatePattern[j].content ||
                oldPattern[j].relativeIndent !== candidatePattern[j].relativeIndent) {
                console.log(`[Python Matcher] Mismatch at line ${j + 1}:`);
                console.log(`  Expected: "${oldPattern[j].content}" (rel indent: ${oldPattern[j].relativeIndent})`);
                if (j < candidatePattern.length) {
                    console.log(`  Got:      "${candidatePattern[j].content}" (rel indent: ${candidatePattern[j].relativeIndent})`);
                }
                else {
 ...
 */
export function applyPythonMatch(fileContent, matchResult) {
    const fileLines = fileContent.split('\n');
...
//# sourceMappingURL=pythonIndentMatcher.js.map