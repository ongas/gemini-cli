...
import { detectIdeFromEnv, IDE_DEFINITIONS, } from '@google/gemini-cli-core/src/ide/detect-ide.js';
 ...
 */
const MANAGED_EXTENSION_SURFACES = new Set([
...
let logger;
let log = () => { };
async function checkForUpdates(context, log, isManagedExtensionSurface) {
    try {
        ...
        if (!isManagedExtensionSurface &&
            latestVersion &&
            semver.gt(latestVersion, currentVersion)) {
            const selection = await vscode.window.showInformationMessage(`A new version (${latestVersion}) of the Gemini CLI Companion extension is available.`, 'Update to latest version');
            if (selection === 'Update to latest version') {
                // The install command will update the extension if a newer version is found.
                await vscode.commands.executeCommand('workbench.extensions.installExtension', CLI_IDE_COMPANION_IDENTIFIER);
            }
        }
    }
    catch (error) {
