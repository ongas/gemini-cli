...
import { CloseDiffRequestSchema, IdeContextNotificationSchema, OpenDiffRequestSchema, } from '@google/gemini-cli-core/src/ide/types.js';
...
async function writePortAndWorkspace({ context, port, portFile, ppidPortFile, authToken, log, }) {
...
function sendIdeContextUpdateNotification(transport, log, openFilesManager) {
...
export class IDEServer {
    ...
    start(context) {
        return new Promise((resolve) => {
            ...
            const sessionsWithInitialNotification = new Set();
            const app = express();
                ...
                if (sessionId && this.transports[sessionId]) {
                    ...
                    const keepAlive = setInterval(() => {
                        ...
                        transport
                            ...
                            .catch((error) => {
                            missedPings++;
                            this.log(`Failed to send keep-alive ping for session ${sessionId}. Missed pings: ${missedPings}. Error: ${error.message}`);
                            if (missedPings >= 3) {
                                this.log(`Session ${sessionId} missed ${missedPings} pings. Closing connection and cleaning up interval.`);
            ...
            this.server = app.listen(0, '127.0.0.1', async () => {
                ...
                if (address && typeof address !== 'string') {
                    ...
                    this.portFile = path.join(os.tmpdir(), `gemini-ide-server-${this.port}.json`);
                    this.ppidPortFile = path.join(os.tmpdir(), `gemini-ide-server-${process.ppid}.json`);
...
const createMcpServer = (diffManager, log) => {
    const server = new McpServer({
        name: 'gemini-cli-companion-mcp-server',
        version: '1.0.0',
    }, { capabilities: { logging: {} } });
    server.registerTool('openDiff', {
        description: '(IDE Tool) Open a diff view to create or modify a file. Returns a notification once the diff has been accepted or rejcted.',
