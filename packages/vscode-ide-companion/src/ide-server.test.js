...
const getPortFromMock = (replaceMock) => {
    const port = vi
        .mocked(replaceMock)
...
describe('IDEServer', () => {
    ...
    it('should set environment variables and workspace path on start with multiple folders', async () => {
        ...
        expect(replaceMock).toHaveBeenNthCalledWith(1, 'GEMINI_CLI_IDE_SERVER_PORT', expect.any(String));
        ...
        const expectedPortFile = path.join('/tmp', `gemini-ide-server-${port}.json`);
        const expectedPpidPortFile = path.join('/tmp', `gemini-ide-server-${process.ppid}.json`);
        ...
        expect(fs.writeFile).toHaveBeenCalledWith(expectedPpidPortFile, expectedContent);
    ...
    it('should update the path when workspace folders change', async () => {
        ...
        expect(fs.writeFile).toHaveBeenCalledWith(expectedPpidPortFile, expectedContent2);
        expect(fs.chmod).toHaveBeenCalledWith(expectedPortFile, 0o600);
        expect(fs.chmod).toHaveBeenCalledWith(expectedPpidPortFile, 0o600);
    });
    it('should clear env vars and delete port file on stop', async () => {
        ...
        const ppidPortFile = path.join('/tmp', `gemini-ide-server-${process.ppid}.json`);
...
//# sourceMappingURL=ide-server.test.js.map