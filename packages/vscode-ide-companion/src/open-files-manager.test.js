...
describe('OpenFilesManager', () => {
  ...
  let onDidDeleteFilesListener;
  let onDidCloseTextDocumentListener;
  let onDidRenameFilesListener;
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  const getUri = (path) => vscode.Uri.file(path);
  const addFile = (uri) => {
  ...
  it('removes a file when it is closed', async () => {
    ...
    await vi.advanceTimersByTimeAsync(100);
    expect(manager.state.workspaceState.openFiles).toHaveLength(1);
    onDidCloseTextDocumentListener({ uri });
    await vi.advanceTimersByTimeAsync(100);
...
//# sourceMappingURL=open-files-manager.test.js.map
